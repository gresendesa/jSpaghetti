(function(){
  ...
  /*Route class*/
  function State(route, shared, lastProcedureRoute, returnToTheLastProcedure){
    this.route = route
    this.shared = shared
    this.lastProcedureRoute = lastProcedureRoute
    this.returnToTheLastProcedure = returnToTheLastProcedure
  }
    ...
    var loop = setInterval(function(){
      if ((!currentSequence.state.route) || ((currentSequence.signalChannel != null) && (jSpaghetti.state.ready))){
        ...
        if (!currentSequence.state.route){
          ...
          currentSequence.state.returnToTheLastProcedure = false
        }
  ...
  /*It runs the especified procedure*/
  function runProcedure(procedureName, moduleName, sequenceName, callNextProcedure){
    var sequence = jSpaghetti.modules[moduleName].sequences[sequenceName]
    sequence.state.shared.$ = jSpaghetti.modules[moduleName].procedures[procedureName](sequence.state.shared, getSharedFunctions(moduleName, sequenceName))
    if (callNextProcedure) sequence.events.dispatchEvent(getEvent(LAST_COMMAND_TERMINATED))
  }
              ...
              setTimeout(function(){
                if (jSpaghetti.state.ready){
                  ...
                  if ((currentSequence.state.route) &&
                    ...
                    //-----Listeners-----//
                    if(!currentSequence.events.isLastCommandTerminatedEventDefined){
                      currentSequence.events.addEventListener(LAST_COMMAND_TERMINATED, function(){ //It listens for last command terminated event
                        currentSequence.events.isLastCommandTerminatedEventDefined = true
                    ...
                    if(currentSequence.state.returnToTheLastProcedure){
                      currentSequence.state.returnToTheLastProcedure = false
                    ...
                    switch(typeof(currentCommand)){
                            ...
                            if (typeof(currentCommand[WAIT_COMMAND]) == 'object'){
                              switch(getFirstAttribName(currentCommand[WAIT_COMMAND])){
                                case WAIT_FOR_THE_SIGNAL_FLAG:
                                  ...
                                  currentSequence.state.returnToTheLastProcedure = true
                                  ...
                                  setTimeout(function(){
                                    runProcedure(currentCommand[WAIT_COMMAND][WAIT_FOR_THE_SIGNAL_FLAG], moduleName, sequenceName, false)
                                  }, 0)
                                  break
                                default: break
                              }
                              ...
                              var loop = setInterval(function(){
                                ...
                                if ((!currentSequence.state.route) ||
                                  ...
                                  currentSequence.events.dispatchEvent(getEvent(LAST_COMMAND_TERMINATED))
                                }
                              }, DEFAULT_DELAY)
                            }
                            break
                      ...
                      default: //Ir handles custom procedure commands and _exit
                        if (currentCommand != EXIT_COMMAND){
                          ...
                          setTimeout(function(){
                            runProcedure(currentCommand, moduleName, sequenceName, true)
                          }, 0)
                        } else {
